...
@Component({
  ...
  templateUrl: './read-serv-muni.component.html',
  styleUrl: './read-serv-muni.component.scss'
})
export class ReadServMuniComponent implements OnInit, AfterViewInit{
  [x: string]: any;
  
    displayedColumns: string[] = ['id','title','description', 'contactinf', 'location', 'image', 'creationdate', 'action'];
    successmsg: any;
    dataSource!: MatTableDataSource<MunicipalServicesData>;
    ...
    deleteID(id:any){
      this.alerboxservice.confirmDialog({
        ...
        message: '¿Esta seguro que desea borrar este servicio publico?',
        comfirmText: 'Si',
        cancelText: 'No'
      }).subscribe(res =>{
        if(res){
            ...
            //console.log(id,'deleteid==>');
              this.apiservice.deleteData(this.table,id).subscribe((res)=>{
                ...
                this.successmsg = "El servicio publico ha sido borrado";
                setTimeout(() => 
                {
