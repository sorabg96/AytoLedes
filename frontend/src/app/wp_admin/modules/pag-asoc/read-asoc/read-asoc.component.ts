...
@Component({
  ...
  templateUrl: './read-asoc.component.html',
  styleUrl: './read-asoc.component.scss'
})
export class ReadAsocComponent implements OnInit, AfterViewInit{
  [x: string]: any;
  
    displayedColumns: string[] = ['id','title','description', 'contactinf', 'location', 'image', 'creationdate', 'action'];
    successmsg: any;
    dataSource!: MatTableDataSource<AsocData>;
    ...
    deleteID(id:any){
      this.alerboxservice.confirmDialog({
        ...
        message: '¿Esta seguro que desea borrar esta asociación?',
        comfirmText: 'Si',
        cancelText: 'No'
      }).subscribe(res =>{
        if(res){
            ...
            //console.log(id,'deleteid==>');
              this.apiservice.deleteData(this.table,id).subscribe((res)=>{
                ...
                this.successmsg = "La asociacion ha sido borrado";
                setTimeout(() => 
                {
