...
export interface SportsAreaData{
  id: number;
  title: string;
  image: string;
  description: string;
...
@Component({
  ...
  templateUrl: './read-sports.component.html',
  styleUrl: './read-sports.component.scss'
})
export class ReadSportsComponent implements OnInit, AfterViewInit{
  [x: string]: any;
  
    displayedColumns: string[] = ['id','title','description', 'contactinf', 'location', 'image', 'creationdate', 'action'];
    successmsg: any;
    dataSource!: MatTableDataSource<SportsAreaData>;
    ...
    deleteID(id:any){
      this.alerboxservice.confirmDialog({
        ...
        message: '¿Esta seguro que desea borrar esta zona deportiva?',
        comfirmText: 'Si',
        cancelText: 'No'
      }).subscribe(res =>{
        if(res){
            ...
            //console.log(id,'deleteid==>');
              this.apiservice.deleteData(this.table,id).subscribe((res)=>{
                ...
                this.successmsg = "La zona deportiva ha sido borrado";
                setTimeout(() => 
                {
